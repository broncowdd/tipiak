...
<?php
	...
	if (!empty($_GET['ext'])){   
		...
		$reg='#(?<=")([^\'"\?]+(\.[a-z0-9]{2,4}))(?=["\?])|(?<=\')([^\'"\?]+(\.[a-z0-9]{2,4}))(?=["\?])|(?<=url\()(.+(\.[a-z0-9]{2,4}))(?=[\)])#i';	
	}
...
if (URL!='None'){
	...
	if ($page=file_curl_contents(URL)){
		...
		if (preg_match_all($reg,$page,$results)){
			...
			echo'<h1>'.count($results).' files tipiakable.<input type="button" onclick="selectall()" value="&#10004;" title="select all"/><input type="button" onclick="unselectall()" value="&#10008;" title="unselect all"/></h1>';
