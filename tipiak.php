		...
		if (preg_match_all($reg,$page,$results)){
			...
			foreach ($results as $key=>$val){
				if ($val=try_autocomplete_relatives_url($val)){
					...
					if (!empty($extension)){
						echo '<li class="'.$extension.'"><a href="'.$val.'" title="Download" download="'.strip_tags(basename($val)).'">'.$extension.'</a> - <input type="checkbox" name="file'.$key.'" id="file'.$key.'" value="'.$val.'"/><label title="select to make a zip" for="file'.$key.'">'.$val.'</label></li>
						';
					}
				}
			}
