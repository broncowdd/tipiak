<?php
	...
	if ($_POST&&count($_POST)>1){
		...
		$filename=$post['zipname'].'.zip';unset($post['zipname']);
		$tozip=array();
...
<head>
	...
	*{box-sizing: border-box}
		...
		li a{display:inline-block;width:64px;text-align:center;font-family:courier;text-decoration:none;border-radius:2px; border:1px solid #888;background-color:#0F0;color:#080;box-shadow:0 1px 2px #040;padding:3px;}
		...
		li.js a{background-color:#aaa;color:#444;box-shadow:0 1px 2px #333;}
...
<?php
	...
	if (!empty($_GET['ext'])){   $regex=strip_tags($_GET['ext']);  }else{ $regex=$listextensions['all files'];	}
	$regex=explode(',',$regex);
	$reg='#(?<=")([^\'"\?]+(';$temp='';
	...
	function try_autocomplete_relatives_url($url){
		if ($url!=''){
			...
			if (isset($data_url['scheme'])&&$data_url['scheme']!=''){
				...
				return $scheme.str_replace('//','/',$host.$data_url['path']);
			}
		}
		return false;
	}
...
if (URL!='None'){
	echo '<form action="#" method="POST"><input type="hidden" name="zipname" value="'.filterchars(URL).'"/>';
	if ($page=file_curl_contents(URL)){
		...
		if (preg_match_all($reg,$page,$results)){
			...
			echo'<h1>'.count($results).' files tipiakable.</h1>';
	
			foreach ($results as $key=>$val){
				if ($val=try_autocomplete_relatives_url($val)){
					$extension=pathinfo($val,PATHINFO_EXTENSION);
					if (!empty($extension)){
						echo '<li class="'.$extension.'"><a href="'.$val.'" title="Download">'.$extension.'</a> - <input type="checkbox" name="file'.$key.'" id="file'.$key.'" value="'.$val.'"/><label title="select to make a zip" for="file'.$key.'">'.$val.'</label></li>
						';
					}
				}
			}
...
<footer>
	...
	<span style="float:right">Cod&#233; &#224; la <em>va-comme-jte-pousse</em> par <a href="http://warriordudimanche.net">Bronco</a></span>
